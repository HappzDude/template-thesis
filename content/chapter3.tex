...
\end{figure}
\subsection{Interoperability}
Grafana can interact withe nearly every Interface to sent alerts to because of its Web hooks which can send and request to an Restful endpoint over a JSON document. Whats special about the alerting engine of Grafana is that Pictures can be send within the alerts. The Pictures is taken of the Grafana to present the alert in a Visual way and can be send with the notification or can be uploaded to Services like Amazon S3.

\subsection{Conclusion}
Grafana is by far the best and biggest Visualization tool on the Open-Source Market. Do to the capability to writhe custom plug ins nearly every tool stack has an integration which is easy to install and uses the features of Grafana in the intention of the developer 
